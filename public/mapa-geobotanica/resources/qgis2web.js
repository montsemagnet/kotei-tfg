...
//initial view - epsg:3857
var initialExtent = [264307.897634, 5155496.929919, 272942.435544, 5160592.695125];
...
fitInitialView();
map.once('postrender', fitInitialView);
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    var popupText = '';
    var pairList = layer.get('popupPairFields') || [];
    var pairByFirst = {};
    var skipPaired = {};
    for (var p = 0; p < pairList.length; p++) {
        var pair = pairList[p];
        if (pair && pair.length >= 2) {
            pairByFirst[pair[0]] = pair[1];
            skipPaired[pair[1]] = true;
        }
    }
    for (var i = 0; i < currentFeatureKeys.length; i++) {
        if (currentFeatureKeys[i] != 'geometry' &&
            ...
            if (skipPaired[currentFeatureKeys[i]]) {
                continue;
            }
            if (pairByFirst[currentFeatureKeys[i]]) {
                var pairRow = '';
                var pairKeys = [currentFeatureKeys[i], pairByFirst[currentFeatureKeys[i]]];
                for (var pk = 0; pk < pairKeys.length; pk++) {
                    var pairKey = pairKeys[pk];
                    if (layer.get('fieldLabels')[pairKey] == "hidden field") {
                        continue;
                    }
                    if (layer.get('fieldLabels')[pairKey] == "inline label - visible with data" &&
                        currentFeature.get(pairKey) == null) {
                        continue;
                    }
                    var pairAlias = layer.get('fieldAliases')[pairKey] || pairKey;
                    var pairVal = currentFeature.get(pairKey);
                    pairRow += '<th>' + pairAlias + '</th><td>' +
                        (pairVal != null ? autolinker.link(pairVal.toLocaleString()) : '') +
                        '</td>';
                }
                if (pairRow) {
                    popupText += '<tr class="popup-pair-row">' + pairRow + '</tr>';
                }
                continue;
            }
            var popupField = '';
