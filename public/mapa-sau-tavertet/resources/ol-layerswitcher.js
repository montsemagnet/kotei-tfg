        ...
        if (lyr instanceof LayerGroup && !lyr.get('combine')) {
            ...
            if (lyr.get('fold')) {
                ...
                label.addEventListener('click', function (e) {
                    LayerSwitcher.toggleFold_(lyr, li);
                    const btn = li.querySelector('button.layer-fold-btn');
                    if (btn) {
                        btn.textContent = lyr.get('fold') === 'open' ? '▼' : '▶';
                    }
                    e.preventDefault();
            ...
            if (legendHtml) {
                ...
                // Clic al nom també obre/tanca la llegenda (com a QGIS)
                label.style.cursor = 'pointer';
                label.title = 'Clic per veure la simbologia';
                label.addEventListener('click', function (e) {
                    // no interferir amb el checkbox (htmlFor)
