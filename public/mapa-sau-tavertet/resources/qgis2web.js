...
(function prepareLayerLegends(layers) {
    layers.forEach(function (lyr) {
        ...
        var t = lyr.get('title');
        if (typeof t === 'string' && /<img/i.test(t)) {
...
// Vista inicial: itinerari Sau–Tavertet (només un cop; després el zoom és lliure)
var __initialViewDone = false;
function fitInitialItinerariView() {
    if (__initialViewDone) {
        map.updateSize();
        return;
    }
    map.updateSize();
    ...
    [lyr_ParadesP_21, lyr_RutaSauTaveret_22].forEach(function (lyr) {
        if (lyr && lyr.getSource && lyr.getSource()) {
            var e = lyr.getSource().getExtent();
            if (e && isFinite(e[0])) {
    ...
    if (!ol.extent.isEmpty(extent)) {
        // Omplir la pantalla amb tot l'itinerari (poc marge blanc)
        ...
        map.getView().fit([265357.868750, 5155175.665657, 271801.841929, 5159691.482684], { size: size });
    }
    map.updateSize();
    __initialViewDone = true;
}

fitInitialItinerariView();
...
// Les capes amb legendHtml tenen ▶/▼ al panell (vegeu ol-layerswitcher.js)
    


map.getView().on('change:resolution', function(evt){

            lyr_Carbesdenivell_15.setStyle(style_Carbesdenivell_15);
            lyr_Contruccions_16.setStyle(style_Contruccions_16);
            lyr_HidrografiaPoligons_17.setStyle(style_HidrografiaPoligons_17);
            lyr_Xarxaviria_18.setStyle(style_Xarxaviria_18);
            lyr_HidrografiaLniesRieresiRius_19.setStyle(style_HidrografiaLniesRieresiRius_19);
});
